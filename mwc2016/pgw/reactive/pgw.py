...
def check_configuration():
    ...
    for k in required_keys:
        if not config(k):
            ...
            return
    status_set('active', '')
    set_state('pgw.configured')


@when('docker.available', 'pgw.configured')
def run_pgw():
    ...
    # Render the template
    render('docker-compose.yml', 'files/pgw/docker-compose.yml', cfg)

    comp = Compose('files/pgw')
    comp.up()
